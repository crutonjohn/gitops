...
<head>
    ...
    <meta name="description" content="<?php echo $antiXss->xss_clean(
      get_config("page_title"),
    ); ?>">
    <title><?php echo $antiXss->xss_clean(get_config("page_title")); ?></title>
    <link rel="icon" href="<?php echo $antiXss->xss_clean(
      get_config("baseurl"),
    ); ?>/favicon.ico" type="image/x-icon">
    <link rel="stylesheet"
          href="<?php echo $antiXss->xss_clean(
            get_config("baseurl"),
          ); ?>/template/<?php echo $antiXss->xss_clean(
  get_config("template"),
  ...
  get_config("template"),
    ...
    <script src="<?php echo $antiXss->xss_clean(
      get_config("baseurl"),
    ); ?>/template/<?php echo $antiXss->xss_clean(
  get_config("template"),
  ...
  get_config("template"),
    ...
    <?php echo !empty(lang("custom_css"))
      ? "<style>" . lang("custom_css") . "</style>"
      : ""; ?>
...
<div class="content1">
    ...
    <nav class="site-nav" id="site-nav">
        <div class="nav-inner">
            <a class="nav-logo" href="./index.php"><?php echo $antiXss->xss_clean(
              get_config("page_title"),
            ); ?></a>
            ...
            <ul class="nav-links" id="nav-links">
                ...
                <?php
                if (!get_config("disable_online_players")) { ?>
                    <li><a onclick="$('#serverstatus').trigger('click'); scrollToContent();"><?php elang(
                      "server_status",
                    ); ?></a></li>
                <?php }
                if (!get_config("disable_top_players")) { ?>
                    <li><a onclick="$('#topplayers').trigger('click'); scrollToContent();"><?php elang(
                      "top_players",
                    ); ?></a></li>
                <?php }
                ?>
    ...
    <div class="nav-mobile" id="nav-mobile">
        ...
        <?php
        if (!get_config("disable_online_players")) { ?>
            <a onclick="$('#serverstatus').trigger('click'); closeMobileNav(); scrollToContent();"><?php elang(
              "server_status",
            ); ?></a>
        <?php }
        if (!get_config("disable_top_players")) { ?>
            <a onclick="$('#topplayers').trigger('click'); closeMobileNav(); scrollToContent();"><?php elang(
              "top_players",
            ); ?></a>
        <?php }
        ?>
    ...
    <div class="container">
        ...
        <section class="hero">
            <div class="hero-card">
                <img class="hero-wow-logo" src="<?php echo $antiXss->xss_clean(
                  get_config("baseurl"),
                ); ?>/template/<?php echo $antiXss->xss_clean(
  get_config("template"),
); ?>/images/wow-logo.png" alt="World of Warcraft">
                <div class="hero-badge">
                    <span class="pulse-dot"></span>
                    Wrath of the Lich King &middot; Patch <?php echo $antiXss->xss_clean(
                      get_config("game_version"),
                    ); ?>
                </div>
                <h1 class="hero-title"><?php echo $antiXss->xss_clean(
                  get_config("page_title"),
                ); ?></h1>
                ...
                <div class="hero-realm">
                    ...
                    <?php elang("realmlist"); ?>: <strong><?php echo get_config(
  "realmlist",
); ?></strong>
                </div>
            </div>
